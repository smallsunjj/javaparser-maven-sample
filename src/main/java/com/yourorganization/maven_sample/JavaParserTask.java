...
public class JavaParserTask {
  ...
  private static final String file_name_1 = "lang\\java\\avro\\src\\test\\java\\org\\apache\\avro\\io\\parsing\\TestResolvingGrammarGenerator.java";
  private static final String file_name_2 = "lang\\java\\avro\\src\\test\\java\\org\\apache\\avro\\TestDataFileMeta.java";
  private static final String file_name_3 = "lang\\java\\ipc\\src\\test\\java\\org\\apache\\avro\\TestProtocolGeneric.java";
  ...
  private void initCombinedSolver() throws IOException {
    ...
    //combinedSolver.add(new JavaParserTypeSolver(new File(src_dir)));
    // add jar solver
    //combinedSolver.add(new JarTypeSolver(jar_dir));
    List<File> jar_files = getJarFiles(new File(jar_dir));
    System.out.println(jar_files.size());
    Collections.sort(jar_files);
...
/*    if (jar_dir != null) {
  ...
  public static void main(String[] args) throws IOException {
    ...
    //TypeSolver symbolSolver = new JavaSymbolSolver(combinedSolver);
    jvp.getParserConfiguration().setSymbolResolver(symbolSolver);

    CompilationUnit cu = jvp.parse(new File(file_name_2)).getResult().get();
    //cu.findAll(MethodCallExpr.class).forEach(mce -> System.out.println(mce.resolve().getQualifiedName()));
    ...
    MethodDeclarationCollector.visit(cu, methodModules);
